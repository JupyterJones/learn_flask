...
def add_echo_and_image_to_video():
    try:
        if request.method == 'POST':
            ...
            shutil.copy('echo_output.mp4', os.path.join('static', unique_save+"use.mp4"))
            # Return the unique filename to the client for download
            USE = unique_save+"use.mp4"
            return render_template('viewing_page.html', video_file=unique_save+"echo_audio.mp4", image_file=unique_save+"input_audio.jpg", video_file2=USE)  
